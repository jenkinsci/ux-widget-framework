...
export function renderFlatPipelineFat() {
  ...
  const stages = [
    ...
    makeNode("Failure", [], Result.failure),
    makeNode("Running", [
      makeNode("Job 1", [], Result.running),
      makeNode("Job 2", [], Result.running),
      makeNode("Job 3", [], Result.running)
    ]),
    makeNode("Queued", [
...
export function renderListenersPipeline() {
  ...
  const stages = [
    ...
    makeNode("Browser Tests", [
      makeNode("Internet Explorer", [], Result.queued),
...
export function renderEdgeCases1() {
  __id = 111;
  const stages1 = [
    makeNode("Alpha", [], Result.skipped),
    makeNode("Bravo", [], Result.success),
    makeNode("Charlie", [], Result.skipped)
  ];

  const stages2 = [
    makeNode("Alpha", [
      makeNode("Delta", [], Result.success),
      makeNode("Echo", [], Result.success),
      makeNode("Foxtrot", [], Result.success)
    ]),
    makeNode("Bravo", [], Result.success),
    makeNode("Charlie", [
      makeNode("Golf", [], Result.success),
      makeNode("Hotel", [], Result.success),
      makeNode("Indigo", [], Result.success)
    ])
  ];

  const stages3 = [
    makeNode("Alpha", [], Result.success),
  ...
  return (
    <div>
      ...
      <PipelineGraph
        stages={stages4}
        selectedStage={stages4[0].children[1].nextSibling!.nextSibling}
      />
...
export function renderMultiParallelPipeline() {
  ...
  const stages = [
    makeNode("Build", [], Result.success),
    makeNode("Test", [
      makeNode("JUnit", [], Result.success),
      makeNode("DBUnit", [], Result.success),
      makeNode("Jasmine", [], Result.success)
    ]),
    makeNode("Browser Tests", [
    ...
    makeNode(
      "Dev",
      [
        makeNode("US-East", [], Result.success),
        makeNode("US-West", [], Result.success),
        makeNode("APAC", [], Result.success)
      ],
      Result.success
    ),
...
export function renderLongNames() {
  ...
  const stages = [
    makeNode(
      "Build something with a long and descriptive name that takes up a shitload of space",
      [],
      Result.success
    ),
    makeNode("Test", [
    ...
    makeNode("Browser Tests", [
      makeNode("Firefox", [], Result.success),
      makeNode(
        "Das komputermaschine ist nicht auf mittengraben unt die gerfingerpoken. Watchen das blinkenlights.",
        [],
        Result.failure
      ),
      makeNode(
        "RubberbabybuggybumpersbetyoudidntknowIwasgoingtodothat",
        [],
        Result.running,
        60
      ),
