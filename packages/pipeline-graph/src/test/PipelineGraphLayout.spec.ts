...
function makeNode(
  ...
  completePercent =
    completePercent ||
    (state == Result.running ? Math.floor(Math.random() * 60 + 20) : 50);
  const id = __id++;
  const title = name;
...
function assertRow(
  row: RowType,
  ...nodesParams: Array<[string, number, number]>
) {
  assert.ok(row, `row exists`);
  assert.equal(
    row.length,
    nodesParams.length,
    `row has ${nodesParams.length} nodes`
  );
...
function assertConnection(
  ...
  for (const compositeConnection of connections) {
    const sourceMatch = compositeConnection.sourceNodes.find(
      node => node.name === sourceName
    );
    const destinationMatch = compositeConnection.destinationNodes.find(
      node => node.name === destinationName
    );
  ...
  assert.fail(
    0,
    1,
    `could not find ${sourceName} --> ${destinationName} connection`
  );
}
...
describe("PipelineGraph", () => {
  describe("layoutGraph", () => {
    ...
    it("lays out a multi-stage parallel graph", () => {
      const stages = [
        ...
        makeNode("Bravo", [
          ...
          makeSequence(
            makeNode("India", [], Result.not_built, "STAGE"),
            makeNode("Juliet", [], Result.not_built, "STAGE")
          )
        ]),
        makeNode("Charlie")
      ];
    ...
    it("lays out a single node parallel graph", () => {
      const stages = [
        makeNode("Build", [], Result.success),
        makeNode("Test", [makeNode("JUnit", [], Result.success, "PARALLEL")]),
        makeNode("Deploy")
      ];
